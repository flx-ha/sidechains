...
DogeChain100Setup::DogeChain100Setup() : TestingSetup(CBaseChainParams::REGTEST)
{
    // Disable default enabled BMM checks for dogechain generation - otherwise
...
//
CBlock DogeChain100Setup::CreateAndProcessBlock(const CScript& scriptPubKey)
{
    const CChainParams& chainparams = Params();
    CBlock block;
...
CBlock DogeChain100Setup::CreateAndProcessBlock(const std::vector<CMutableTransaction>& vtx, const CScript& scriptPubKey)
{
    const CChainParams& chainparams = Params();
    CBlock block;
...
CScript DogeChain100Setup::GetCoinbaseScript() const
{
    return CScript() << ToByteVector(coinbaseKey.GetPubKey()) << OP_CHECKSIG;
}

DogeChain100Setup::~DogeChain100Setup()
{
}
